...
async function updateMenuTotal(event) {
    ...
    if (!primeroId || !segundoId || !postreId) {
        const menu = await strapi.db.query("api::menu-diario.menu-diario").findOne({
        ...
        if (primeroId === segundoId || primeroId === postreId || segundoId === postreId) {
            throw new errors.ValidationError("Los platos en el menú deben ser únicos");
        }
