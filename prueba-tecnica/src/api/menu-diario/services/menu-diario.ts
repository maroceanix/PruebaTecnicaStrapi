...
export default factories.createCoreService(
    ...
    ({ strapi }) => ({
        async calcularSumaBase(primerId, segundoId, postreId) {
            const primero = await strapi.db.query("api::plato.plato").findOne({
                where: { id: primerId },
            });

            const segundo = await strapi.db.query("api::plato.plato").findOne({
                where: { id: segundoId },
            });
        ...
        async agregarImpuesto(documentIdd, impuestoPorcentaje) {
            const menu = await strapi.documents("api::menu-diario.menu-diario").findOne({
            ...
            const precioConImpuesto = sumaBase * (1 + impuestoPorcentaje / 100);

            return await strapi.documents("api::menu-diario.menu-diario").update({
                documentId: documentIdd,
                data: {
            ...
            const menusFiltrados = menus.filter((menu) => {
                ...
                return !platos.some((plato) =>
                    plato?.alergenos?.some((alergeno) => {
                        ...
                        return alergenosExcluir.includes(nombreAlergeno);
                    }),
                );
            });
